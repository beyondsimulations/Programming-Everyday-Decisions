...
- Lecture based on the book [Algorithms to live by]{.highlight}[^1]
...
[^1]: With a large number of candidates. With a small number of candidates, we can do even better.
...
[Question:]{.question} What if we don't have a fixed number of candidates, but rather a [fixed amount of time]{.highlight}? 
...
[^2]: An increase in occupancy from 90 to 95% doubles the search time for all drivers!

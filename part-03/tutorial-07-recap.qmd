---
title: "Tutorial III.I - Caching and Recap"
...
4. How many times you've ordered it in a variable `num_orders` (as an integer)
...
2. Use at least **two conditions** based on the song's play count and last played time  (`play_count` and `last_played_minutes`)
...
assert isinstance(last_played_minutes, int), "last_played_minutes should be an integer"
assert isinstance(should_evict, bool), "should_evict should be a boolean"
```

# Section 3 - Lists and Tuples

Lists work like a cache's storage system, holding multiple items that we might need later. Tuples are similar, but they are immutable. This means that once a tuple is created, its contents cannot be changed.
...
::: {.callout-tip}
This would **not have been possible with a tuple**, as we cannot change the contents of a tuple once it is created!
:::
...
2. If the site has been visited **less than 5 times**, remove it from the cache
...
#| code-fold: true
# Assertions to verify your solution
assert len(browser_history) == 3, "browser_history should have 3 items"
